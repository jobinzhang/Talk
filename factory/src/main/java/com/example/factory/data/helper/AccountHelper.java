...
public class AccountHelper {
    ...
    public static void register(final RegisterModel model, final DataSource.Callback<UserCard> callback) {
        ...
        // 得到一个Call
        Call<RspModel<UserCard>> call = service.accountRegister(model);
        // 异步的请求
    ...
    public static void login(final LoginModel model, final DataSource.Callback<UserCard> callback) {
        ...
        RemoteService service = Network.remote();
        // 得到一个Call
        Call<RspModel<UserCard>> call = service.accountLogin(model.getAccount(), model.getPassword());
        // 异步的请求
        call.enqueue(new AccountRspCallback(callback));
    }
    ...
    public static void bindPush(final DataSource.Callback<UserCard> callback) {
        ...
        // 调用Retrofit对我们的网络请求接口做代理
        RemoteService service = Network.remote();
        Call<RspModel<UserCard>> call = service.accountBind(pushId);
        call.enqueue(new AccountRspCallback(callback));
    }
     ...
     * 请求的回调部分封装
     */
    private static class AccountRspCallback implements Callback<RspModel<UserCard>> {
